...
def signup(body: UserRegister, db: Session = Depends(get_db)):
    if crud.get_user_by_email(db, body.username):
    ...
    try:
        user = crud.create_user(db, body.username, hashed, body.nickname,
                                body.business_name, body.business_type,
                                body.phone, body.user_email)
...
async def request_password_reset(email: str, db: Session = Depends(get_db)):
    ...
    reset_link = f"https://dsic.duckdns.org/ownerbot/reset-password?token={token}"
    ...
    message = MessageSchema(
        ...
        <a href="{reset_link}" style="
            ...
            background-color:#0F1E3F;color:white;
