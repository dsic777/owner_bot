...
| 플랫폼 | 모바일 웹 (Chrome 브라우저, PWA 설치 차단) |
...
| AI | Claude API (claude-haiku-4-5-20251001) | 콘텐츠 생성·업종 감지, 속도·비용 최적화 |
| 인증 | JWT (python-jose) | Stateless, 모바일 호환 |
| CSS | Tailwind CSS | 유틸리티 우선, 빠른 모바일 스타일링 |
| 배포 | Docker + EC2 + Nginx | 기존 portfolio 서버 재활용 |
| PWA | ~~Web App Manifest~~ → 차단 | 구 도메인 혼동·오작동 이슈로 설치 비활성화 |
...
│   │   │   ├── crud.py
│   │   │   └── service.py
│   │   ├── generate/            # AI 콘텐츠 생성 (claude-haiku)
│   │   ├── history/             # 생성 이력·크레딧 거래
│   │   ├── mypage/              # 마이페이지·크레딧 충전
│   │   └── business_type/       # 업종 목록·AI 감지 (v2 DB 연동)
│   ├── requirements.txt
...
│   │       └── MyPage.tsx
│   ├── public/
│   │   ├── manifest.json        # PWA 설정 (비활성 — manifest 링크 제거됨)
│   │   ├── sw.js                # 서비스워커 (기존 설치 해제용)
│   │   ├── mic.png              # 마이크 아이콘 (v2에서 이식)
│   │   ├── icon.png             # 앱 아이콘
│   │   ├── main.png             # 랜딩 아이콘 이미지 (4종 서비스)
│   │   └── bottom.png           # 야경 건물 하단 이미지
...
### 업종 `/api/business-types`

| 메서드 | 경로 | 설명 |
|--------|------|------|
| GET | / | 업종 목록 조회 (v2 ownerbot2 DB) |
| POST | /detect | 상호명으로 업종 자동 감지 (aliases → Claude AI 순) |
...
### 2026-05-05 — 콘텐츠 생성 페이지 전면 개편 + PWA 차단

#### 완료 내용

**콘텐츠 생성 페이지 (GeneratePage) 전면 개편**
- 타이틀·문구: 팀 프로젝트(ss2) 기준으로 통일
- 디자인·기능: v2(ss3) 기준으로 적용
- 업종 선택: native `<select>` → 커스텀 드롭다운 (직접입력 최상단 + 가나다 정렬)
- 업종 DB: v2 ownerbot2 DB에서 실시간 로드 (`V2_DATABASE_URL` 환경변수)
- AI 업종 자동 감지: 상호명 입력 900ms 후 aliases 매칭 → Claude Haiku 순으로 감지
- 카카오 분류 피커: 그룹→세부 2단계 선택 UI 추가
- 톤 선택: `<select>` → 3개 버튼 (친근하게/전문적으로/감성적으로)
- 마이크 아이콘: 이모지 🎤 → mic.png 이미지 (v2에서 이식), 녹음 중 빨간색 필터
- 생성 버튼명: "마케팅 콘텐츠 생성하기"
- 복사 버튼: 탭별 클립보드 복사 기능
- 크레딧 소진 모달: 패키지 선택 충전 UI

**백엔드 — 업종 모듈 신규 추가**
- `modules/business_type/` 모듈 생성
- v2 ownerbot2 DB 전용 엔진 (lazy init, `V2_DATABASE_URL`)
- `GET /api/business-types` — is_active 업종 목록
- `POST /api/business-types/detect` — 상호명 AI 감지
- `config.py`에 `V2_DATABASE_URL` 환경변수 추가

**폰트 1레벨 상향**
- 결과 탭 바: `text-base` → `text-lg`
- 블로그 제목: `text-xl` → `text-2xl`
- 블로그 본문·해시태그: `text-base` → `text-lg`
- 업종 드롭다운 목록: `text-lg`
- 레이블 클래스: `text-lg`

**PWA 설치 차단**
- `index.html`: `<link rel="manifest">` + iOS PWA 메타태그 4줄 제거
- `main.tsx`: 서비스워커 등록 → 기존 설치된 SW 자동 해제로 변경
- 이유: 구 도메인(`dsic.duckdns.org`)으로 설치된 PWA가 오작동, 일관성 없는 화면 노출
...
> 최종 업데이트: 2026-05-05 | 작성: 유동주 + Claude AI (claude-sonnet-4-6)
